 ...
 */
function collectAllLibPins(symbol: schematicProto.I_LibSymbol): schematicProto.I_Pin[] {
    const result: schematicProto.I_Pin[] = [];

    function parseUnitStyleFromName(name: string): { unit: number; style: number } {
...
export function renderSymbol(symbol: schematicProto.I_LibSymbol, options: RenderOptions = {}) {
    ...
    return renderSchematic({
        ...
        symbols: [{
            ...
            instances: { projects: [] },
        }], wires: [], buses: [], bus_entries: [], bus_aliases: [], junctions: [],
