...
class SDFGeometryGenerator {
    ...
    generate(
        res = 64,
        distFunc = "float dist( vec3 p ){ return length(p) - 0.5; }",
        bounds = 1,
    ) {
        let w, h;
        if (res == 8) [w, h] = [32, 16];
            ...
            throw new Error(
                "THREE.SDFGeometryGenerator: Resolution must be in range 8 < res < 1024 and must be ^2",
            );
        ...
        if (w > maxTexSize || h > maxTexSize)
            throw new Error(
                "THREE.SDFGeometryGenerator: Your device does not support this resolution ( " +
                    res +
                    " ), decrease [res] param.",
            );
        ...
        const sdfRT = this.computeSDF(
            w,
            h,
            tilesX,
            tilesY,
            bounds,
            sdfCompute.replace("[#dist#]", distFunc),
        );
    ...
    computeSDF(width, height, tilesX, tilesY, bounds, shader) {
        ...
        while (currentTile++ < tiles) {
            ...
            const [px, py] = [
                tileSize / 2 + (c % tilesX) * tileSize - width / 2,
                tileSize / 2 + Math.floor(c / tilesX) * tileSize - height / 2,
            ];
            const compPlane = new Mesh(
                geometry,
                new ShaderMaterial({
                    uniforms: {
