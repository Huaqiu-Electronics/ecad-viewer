...
class Volume {
    ...
    getData(i, j, k) {
        return this.data[
            k * this.xLength * this.yLength + j * this.xLength + i
        ];
    }
    ...
    map(functionToMap, context) {
        ...
        for (let i = 0; i < length; i++) {
            this.data[i] = functionToMap.call(
                context,
                this.data[i],
                i,
                this.data,
    ...
    extractPerpendicularPlane(axis, RASIndex) {
        ...
        switch (axis) {
            case "x":
                ...
                planeMatrix.setPosition(
                    new Vector3(RASIndex - positionOffset, 0, 0),
                );
                break;
            case "y":
                axisInIJK.set(0, 1, 0);
                ...
                planeMatrix.setPosition(
                    new Vector3(0, RASIndex - positionOffset, 0),
                );
                break;
            case "z":
            default:
                ...
                planeMatrix.setPosition(
                    new Vector3(0, 0, RASIndex - positionOffset),
                );
                break;
        }
        ...
        IJKIndex = Math.abs(
            Math.round(
                IJKIndex.applyMatrix4(volume.inverseMatrix).dot(axisInIJK),
            ),
        );
        const base = [
            new Vector3(1, 0, 0),
