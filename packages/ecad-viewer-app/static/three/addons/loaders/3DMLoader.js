    ...
    _createMaterial(material, renderEnvironment) {
        ...
        for (let i = 0; i < material.textures.length; i++) {
            ...
            if (texture.image !== null) {
                ...
                map.wrapS =
                    texture.wrapU === 0 ? RepeatWrapping : ClampToEdgeWrapping;
                map.wrapT =
                    texture.wrapV === 0 ? RepeatWrapping : ClampToEdgeWrapping;
                    ...
                    switch (attributes.materialSource.name) {
                        case "ObjectMaterialSource_MaterialFromLayer":
                            ...
                            if (attributes.layerIndex >= 0) {
                                matId =
                                    data.layers[attributes.layerIndex]
                                        .renderMaterialIndex;
                            } else {
                                matId = null;
                            }
...
function Rhino3dmWorker() {
    ...
    function decodeObjects(rhino, buffer) {
        ...
        for (let i = 0; i < doc.materials().count; i++) {
            ...
            if (material.pbrSupported) {
                textures.push(
                    ...extractTextures(_material, pbrTextureTypes, doc),
                );
                ...
                case "environment":
                    ...
                    for (let j = 0; j < doc.embeddedFiles().count; j++) {
                        ...
                        if (fileName === _fileName) {
                            const background =
                                doc.getEmbeddedFileAsBase64(fileName);
        ...
        const renderSettings = {
            ...
            backgroundColorTop: doc.settings().renderSettings()
                .backgroundColorTop,
            ...
            renderAnnotations: doc.settings().renderSettings()
                .renderAnnotations,
            ...
            safeFrame: extractProperties(
                doc.settings().renderSettings().safeFrame,
            ),
            dithering: extractProperties(
                doc.settings().renderSettings().dithering,
            ),
            skylight: extractProperties(
                doc.settings().renderSettings().skylight,
            ),
        ...
        if (geometry) {
            ...
            if (_attributes.decals().count > 0) {
                self.postMessage({
                    ...
                    data: {
                        message:
                            "THREE.3DMLoader: No conversion exists for the decals associated with this object.",
                        type: "no conversion",
                        guid: _attributes.id,
                    },
                });
