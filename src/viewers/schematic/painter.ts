...
class ImagePainter extends SchematicItemPainter {
    classes = [schematic_items.Image];

    layers_for(item: schematic_items.Image) {
        return [LayerNames.bitmap];
    }

    paint(layer: ViewLayer, t: schematic_items.Image) {
        const img = t.img;
