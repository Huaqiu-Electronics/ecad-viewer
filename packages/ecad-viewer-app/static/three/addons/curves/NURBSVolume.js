...
class NURBSVolume {
    constructor(
        ...
        knots1,
        knots2,
        knots3 /* arrays of reals */,
        controlPoints /* array^3 of Vector(2|3|4) */,
    ) {
    ...
    getPoint(t1, t2, t3, target) {
        ...
        const v =
            this.knots2[0] +
            t2 * (this.knots2[this.knots2.length - 1] - this.knots2[0]); // linear mapping t2->v
        const w =
            this.knots3[0] +
            t3 * (this.knots3[this.knots3.length - 1] - this.knots3[0]); // linear mapping t3->w
