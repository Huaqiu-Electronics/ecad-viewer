...
export class LibSymbolPinPainter extends SchematicItemPainter {
    ...
    paint(layer: ViewLayer, p: schematic_items.LibSymbolPin) {
        if (p.definition.hide) {
            return;
        }

        const pin: LibSymbolPinInfo = {
            pin: p,
            def: p.definition,
        ...
        LibSymbolPinPainter.apply_symbol_transformations(
            pin,
     ...
     */
    static rotate(pin: LibSymbolPinInfo, center: Vec2, ccw = false) {
    ...
    static mirror_horizontally(pin: LibSymbolPinInfo, center: Vec2) {
    ...
    static mirror_vertically(pin: LibSymbolPinInfo, center: Vec2) {
     ...
     */
    draw_pin_shape(gfx: Renderer, pin: LibSymbolPinInfo) {
     ...
     */
    draw_name_and_number(gfx: Renderer, pin: LibSymbolPinInfo) {
        const def = pin.def;
        const libsym = pin.pin.parent;
...
export type LibSymbolPinInfo = {
    pin: schematic_items.LibSymbolPin;
